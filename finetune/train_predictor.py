...
def create_dataloaders(config: dict, rank: int, world_size: int):
    ...
    num_workers=config.get('num_workers', 2), pin_memory=torch.cuda.is_available(), drop_last=True
    )
    val_loader = DataLoader(
    valid_dataset, batch_size=config['batch_size'], sampler=val_sampler,
    num_workers=config.get('num_workers', 2), pin_memory=torch.cuda.is_available(), drop_last=False
    )
...
def train_model(model, tokenizer, device, config, save_dir, logger, rank, world_size):
    ...
    for epoch_idx in range(config['epochs']):
        ...
        for i, (batch_x, batch_x_stamp) in enumerate(train_loader):
            ...
            loss, s1_loss, s2_loss = model_ref.head.compute_loss(logits[0], logits[1], token_out[0], token_out[1])
        ...
        with torch.no_grad():
            for batch_x, batch_x_stamp in val_loader:
                ...
                logits = model(token_in[0], token_in[1], batch_x_stamp[:, :-1, :])
                val_loss, _, _ = model_ref.head.compute_loss(logits[0], logits[1], token_out[0], token_out[1])
...
def main(config: dict):
    ...
    # Model Initialization (SEQUOIA=1 时加载扩展 tokenizer；predictor 本身结构不变)
    ...
    if rank == 0:
        print(f"Predictor Model Size: {get_model_size(model.module if use_ddp else model)}")
...
if __name__ == '__main__':
    # GPU 训练需用 torchrun 启动；CPU 单进程可直接 `python` 运行
